...
class Controller {
  ...
  addToNetwork(peerId, siteId, status = true, broadcast = true) {
  ...
  findNewTarget() {
    ...
    const possibleTargets = unconnected.filter(obj => {
      return obj.peerId !== this.peerId && obj.active === true;
    });
  ...
  handleSync(syncObj) {
    ...
    syncObj.network.forEach(obj => {
      this.addToNetwork(obj.peerId, obj.siteId, obj.active, false)
    });
