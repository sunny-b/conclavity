...
class View extends EventEmitter {
  ...
  addIcons() {
    const minimize = this.icons.minus.toSvg({ 'stroke-width': 2, color: 'white', class: 'minimize' });
    ...
    if (this.options.showPeers) {
      const collapse = this.icons['chevrons-left'].toSvg({ 'stroke-width': 2, color: 'rgb(17, 117, 232)', class: 'toggle-minus' });
      const expand = this.icons['chevrons-right'].toSvg({ 'stroke-width': 2, color: 'rgb(17, 117, 232)', class: 'toggle-plus' });
      $('.peer-toggle').append(collapse, expand);
