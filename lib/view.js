...
class View extends EventEmitter {
  ...
  addIcons() {
    const whiteMinimize = this.icons.minus.toSvg({ 'stroke-width': 2, color: 'white', class: 'minimize' });
    const exit = this.icons.x.toSvg({ 'stroke-width': 2, color: 'white', class: 'exit' });
    const copy = this.icons.copy.toSvg({ color: 'rgb(17, 117, 232)', class: 'copy-link' });

    $('.video-bar').append(whiteMinimize, exit);
    $('.copy-container').append(copy);

    if (this.options.showPeers) {
      const blueMinimize = this.icons['minus-circle'].toSvg({ 'stroke-width': 2, color: 'rgb(17, 117, 232)', class: 'toggle-minus' });
      const bluePlus = this.icons['plus-circle'].toSvg({ 'stroke-width': 2, color: 'rgb(17, 117, 232)', class: 'toggle-plus' });
      $('.peer-toggle').append(blueMinimize, bluePlus);
  ...
  attachPeerListEvents() {
    $('.peer-toggle').click((e) => {
      $('.peer-toggle').toggleClass('show');
      $('#peerId').toggleClass('disappear');
    });
  }
